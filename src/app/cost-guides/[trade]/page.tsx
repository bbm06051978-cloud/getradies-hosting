...
import { getTradeGuide, tradeGuides } from "../data";
import {
  CheckCircle, Zap, ChevronRight, DollarSign,
  Clock, MapPin, TrendingUp, ShieldCheck,
} from "lucide-react";

export async function generateStaticParams() {
  return tradeGuides.map(t => ({ trade: t.slug }));
}

export async function generateMetadata({ params }: { params: { trade: string } }): Promise<Metadata> {
  const guide = getTradeGuide(params.trade);
  if (!guide) return {};
  return {
    title: `${guide.name} Cost Guide Australia 2024 — GeTradie`,
    description: `How much does a ${guide.name.toLowerCase()} cost in Australia? Average prices from $${guide.avgMin} to $${guide.avgMax}. Compare quotes from verified local tradies.`,
  };
}

export default function TradeCostGuidePage({ params }: { params: { trade: string } }) {
  const guide = getTradeGuide(params.trade);
  if (!guide) notFound();
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
              ...
              <p className="text-sm text-gray-500 mb-4">
                Tell us exactly what you need and get a personalised price estimate for your location — free and instant.
              </p>
              <Link href={`/?job=${guide.name.toLowerCase()}`}>
      ...
      <section className="py-14 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-xs text-gray-400 mt-3 flex items-center gap-1">
            <TrendingUp size={12} />
            Prices are estimates based on Australian market rates. Actual costs may vary by location and job complexity.
          </p>
        </div>
      </section>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 leading-relaxed">
                {guide.name} costs vary significantly across Australia. <strong>Sydney and Melbourne</strong> are typically 15–20% higher than other cities. <strong>Regional and rural areas</strong> may have fewer tradies available, which can also increase prices. Use GeTradie to get quotes from local tradies in your area.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-14 px-4 bg-blue-900">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-blue-200 mb-8">
            Get free quotes from verified local {guide.name.toLowerCase()} professionals on GeTradie. Compare prices, read reviews and hire with confidence.
          </p>
