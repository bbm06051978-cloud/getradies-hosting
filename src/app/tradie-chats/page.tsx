  ...
  useEffect(() => {
    ...
    if (jobId && receiverId && receiverName) {
      setSelectedConv({
        ...
        otherUser: { id: receiverId, name: receiverName, role: "HOMEOWNER" },
        lastMessage: "",
        ...
        <div className="flex flex-1 overflow-hidden" style={{ height: "calc(100vh - 64px)" }}>
          ...
          <div className="w-80 bg-white border-r border-gray-100 flex flex-col flex-shrink-0">
            <div className="p-4 border-b border-gray-100">
              ...
              <div className="flex items-center border border-gray-200 focus-within:border-orange-400 rounded-xl px-3 py-2 gap-2 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="text-center py-12 px-4">
                  ...
                  <p className="text-gray-400 text-xs mt-1">Messages from homeowners appear here</p>
                </div>
              ) : (
                filtered.map(conv => (
                  <button key={conv.jobId} onClick={() => setSelectedConv(conv)}
                    className={`w-full text-left px-4 py-3 border-b border-gray-50 hover:bg-orange-50 transition-colors ${
                      selectedConv?.jobId === conv.jobId ? "bg-orange-50 border-l-4 border-l-orange-500" : ""
                    }`}>
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 bg-blue-900 rounded-full flex items-center justify-center flex-shrink-0">
                        <span className="text-white font-bold text-sm">{conv.otherUser.name.charAt(0).toUpperCase()}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-orange-600 font-medium truncate">{conv.job.title}</p>
            ...
            {!selectedConv ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-gray-400 text-sm max-w-xs">
                  Select a conversation to reply, or wait for homeowners to message you.
                </p>
              </div>
            ) : (
              <>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-400">
                      ...
                      {selectedConv.job.trade && (
                        <span className="bg-orange-100 text-orange-700 font-semibold px-1.5 py-0.5 rounded-full">
                          {selectedConv.job.trade}
                        </span>
                      )}
                    </div>
                ...
                <div className="bg-white border-t border-gray-100 p-4">
                  <div className="flex items-end gap-3">
                    <div className="flex-1 flex items-end border border-gray-200 focus-within:border-orange-400 rounded-2xl px-4 py-3 gap-2 transition-colors bg-slate-50">
