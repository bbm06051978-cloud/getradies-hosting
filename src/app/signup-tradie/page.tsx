...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setServerError("");
    if (!agreedToTerms) { setTermsError("You must agree to the Terms of Service and Privacy Policy."); return; }
    setTermsError("");
    if (!validate()) return;
    setLoading(true);
    try {
      ...
      if (data.success) {
        setSuccess(true);
        if (data.requiresVerification) { window.location.href = `/verify-email?email=${encodeURIComponent(email)}`; } else { window.location.href = tab === "tradie" ? "/tradie-jobs" : "/dashboard"; }
      } else {
