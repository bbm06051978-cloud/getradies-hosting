...
type BookingRef = { id: string; status: string; scheduledAt: string };
...
export default function MyJobsPage() {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case "QUOTED": return { bg: "bg-orange-100 text-orange-700", icon: MessageCircle, label: "Quoted" };
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm mt-0.5">
                  All jobs you have posted on GeTradie
                </p>
              </div>
            </div>
            <Link href="/post-job">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filtered.map((job) => {
                ...
                const Icon = s.icon;
                const pendingQuotes = job.quotes.filter((q) => q.status === "PENDING").length;
                const acceptedQuote = job.quotes.find((q) => q.status === "ACCEPTED");
                const canCancel = ["OPEN", "QUOTED"].includes(job.status);
                ...
                return (
                  <motion.div
                    ...
                    className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:border-blue-200 transition-all"
                  >
                      ...
                      <div className="flex items-start gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${s.bg}`}>
                              <Icon size={11} />
                              {s.label}
                            </span>
                          </div>
                          ...
                          {job.aiEstimate && (
                            <div className="flex items-center gap-1.5 mt-2">
                              ...
                              <span className="text-xs text-blue-600 font-medium">
                                {job.aiEstimate.split("\n").find((l) => l.startsWith("💰"))?.replace("💰", "").trim() || "AI estimate available"}
                              </span>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="flex flex-col items-end gap-2 flex-shrink-0">
                        ...
                        {job.quotes.length > 0 && (
                          <div className="text-right">
                            <p className="text-lg font-bold text-gray-900">{job.quotes.length}</p>
                            <p className="text-xs text-gray-400">quote{job.quotes.length > 1 ? "s" : ""}</p>
                          </div>
                        )}

                        {/* Accepted quote amount */}
                        {acceptedQuote && (
                          <div className="bg-green-50 border border-green-200 rounded-xl px-3 py-1.5 text-right">
                            <p className="text-sm font-bold text-green-700">
                              ${acceptedQuote.amount.toLocaleString()}
                            </p>
                    ...
                    <div className="flex items-center gap-3 mt-4 pt-4 border-t border-gray-100 flex-wrap">
                      ...
                      {job.quotes.length > 0 && (
                        <Link href="/my-quotes">
                          <button className="flex items-center gap-1.5 bg-blue-900 hover:bg-blue-800 text-white px-4 py-2 rounded-xl text-xs font-semibold transition-colors">
                            <MessageCircle size={13} />
                            View {pendingQuotes > 0 ? `${pendingQuotes} New` : ""} Quote{job.quotes.length > 1 ? "s" : ""}
                      ...
                      {job.bookings.length > 0 && (
                        <Link href="/bookings">
                          <button className="flex items-center gap-1.5 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-xl text-xs font-semibold transition-colors">
                            <Calendar size={13} />
                            View Booking
                            <ChevronRight size={13} />
                          </button>
                      ...
                      {/* No quotes yet */}
                      {job.quotes.length === 0 && job.status === "OPEN" && (
                        <span className="text-xs text-gray-400 italic">
                          Waiting for quotes from tradies...
                        </span>
                      )}
                      ...
                      {canCancel && (
                        <button
                          ...
                          className="ml-auto flex items-center gap-1.5 border border-red-200 hover:border-red-400 text-red-500 hover:text-red-700 px-4 py-2 rounded-xl text-xs font-semibold transition-colors"
                        >
                          <XCircle size={13} />
